...
def transform_guidance(latents, batch, sub_timesteps, noise_scheduler, unet, prompt_embeds, class_labels,
    ...
    channel_noise_bias = channel_noise_bias.to(device)
...
def main(args):
    ...
    # print(unet)
    # 计算unet模型大小 获取模型的输入和输出层尺寸
    total_params = sum(p.numel() for p in unet.parameters())
    model_size_in_mb = total_params * 4 / (1024 ** 2)
    # print(f"模型的大小: {model_size_in_mb:.2f} MB")
    # exit()
    if hasattr(unet, 'conv_in'):
        print(f"输入层尺寸: {unet.conv_in.weight.shape}")
    if hasattr(unet, 'conv_out'):
        print(f"输出层尺寸: {unet.conv_out.weight.shape}")

    # print(vae)
    # 计算vae模型大小 获取模型的输入和输出层尺寸
    total_params = sum(p.numel() for p in vae.parameters())
    model_size_in_mb = total_params * 4 / (1024 ** 2)
    # print(f"模型的大小: {model_size_in_mb:.2f} MB")
    # if hasattr(vae, 'conv_in'):
    #     print(f"输入层尺寸: {unet.conv_in.weight.shape}")
    # if hasattr(vae, 'conv_out'):
    #     print(f"输出层尺寸: {unet.conv_out.weight.shape}")
    # exit()
        ...
        for image_i in range(args.first_image_index, args.num_images_per_prompt):
            ...
            with (accelerator.accumulate([unet])):
                ...
                if args.text_to_img:
                    ...
                    #原
                    # model_input = torch.stack([to_tensor(img) for img in batch["pil_images"]]).to(device=device, dtype=weight_dtype)
                    # 先调整图像大小
                    # resize_transform = transforms.Resize((64, 64))
                    # model_input = torch.stack([resize_transform(img) for img in model_input])
                    # 扩展通道以适应4个通道的输入要求
                    # empty_channel = torch.zeros_like(model_input[:, :1, :, :])  # 创建一个全零的通道
                    # model_input = torch.cat([model_input, empty_channel], dim=1)


                    #对比
                    # model_input_latent = batch["image_latents"].to(dtype=weight_dtype)
                    # model_input_pil = torch.stack([to_tensor(img) for img in batch["pil_images"]]).to(device=device, dtype=weight_dtype)

                    # 打印 model_input_latent 的基本信息
                    # print("model_input_latent:")
                    # print(f"Shape: {model_input_latent.shape}")
                    # print(f"Dtype: {model_input_latent.dtype}")
                    # print(f"Device: {model_input_latent.device}")
                    # print(f"Min value: {model_input_latent.min().item()}")
                    # print(f"Max value: {model_input_latent.max().item()}")
                    # print(f"Mean value: {model_input_latent.mean().item()}")

                    # 打印 model_input_pil 的基本信息
                    # print("\nmodel_input_pil:")
                    # print(f"Shape: {model_input_pil.shape}")
                    # print(f"Dtype: {model_input_pil.dtype}")
                    # print(f"Device: {model_input_pil.device}")
                    # print(f"Min value: {model_input_pil.min().item()}")
                    # print(f"Max value: {model_input_pil.max().item()}")
                    # print(f"Mean value: {model_input_pil.mean().item()}")
                    ...
                    if args.offset_noise:
                        ...
                        noise = (torch.randn_like(model_input)*noise_shift).to(dtype=weight_dtype)
                ...
                with torch.no_grad():
                    ...
                    # print(f"Min value: {latents.min().item()}")
                    # print(f"Max value: {latents.max().item()}")
                    # print(f"Mean value: {latents.mean().item()}")
